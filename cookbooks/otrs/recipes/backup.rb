...
azati_folder_backup node[:otrs][:dir] do
  tofile "/mnt/backup/otrs.tar.gz"
  action :execute
end
...
execute "pack_backup" do
  cwd "/mnt/backup"
  command "tar -cf backup.tar db.sql.gz otrs.tar.gz metadata.yml"
end
...
file "/mnt/backup/otrs.tar.gz" do
  action :delete
  backup false
end
