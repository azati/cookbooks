...
azati_folder_backup "#{node[:tomcat6][:catalina_base]}/webapps/openbravo" do
...
azati_folder_backup node[:openbravo][:src_path] do
  tofile "/mnt/backup/openbravo_src.tar.gz"
  action :execute
end
...
execute "pack_backup" do
  cwd "/mnt/backup"
  command "tar -cf backup.tar db.sql.gz openbravo.tar.gz openbravo_att.tar.gz openbravo_src.tar.gz metadata.yml"
end
...
file "/mnt/backup/openbravo_src.tar.gz" do
  action :delete
  backup false
end
