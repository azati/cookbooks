...
azati_folder_restore "/mnt/restore/egroupware_files.tar.gz" do
  todir node[:egroupware][:files_path]
  action :execute
end

azati_folder_restore "/mnt/restore/egroupware_backup.tar.gz" do
  todir node[:egroupware][:backup_path]
  action :execute
end
